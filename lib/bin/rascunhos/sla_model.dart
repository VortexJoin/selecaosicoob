          ...
          child: SfCartesianChart(
            ...
            series: <ChartSeries>[
              ...
              ColumnSeries<SLA, String>(
                ...
                xValueMapper: (SLA sla, _) =>
                    '${sla.data.day}/${sla.data.month}/${sla.data.year}',
                yValueMapper: (SLA sla, _) => sla.percentualDeResposta,
                name: 'SLA',
                enableTooltip: true,
              ),
              ColumnSeries<SLA, String>(
                ...
                xValueMapper: (SLA sla, _) =>
                    '${sla.data.day}/${sla.data.month}/${sla.data.year}',
                yValueMapper: (SLA sla, _) => sla.percentualDeSolucao,
                name: 'SLA',
                enableTooltip: true,
              ),
            ],
...
class SLA {
  ...
  Duration? tempoMaximoResposta;
  Duration? tempoMaximoSolucao;

  DateTime? horaInicioExpediente;
  DateTime? horaFimExpediente;

  final DateTime padraoInicioAtendimento = DateTime(0, 0, 0, 8);
  final DateTime padraoFimoAtendimento = DateTime(0, 0, 0, 17);

  final Duration padraoTempoMaximoResposta = const Duration(hours: 2);
  final Duration padraoTempoMaximoSolucao = const Duration(hours: 8);

  SLA({
    this.horaInicioExpediente,
    this.horaFimExpediente,
    this.tempoMaximoResposta,
    this.tempoMaximoSolucao,
    required this.data,
    ...
    horaInicioExpediente ??= padraoInicioAtendimento;
    horaFimExpediente ??= padraoFimoAtendimento;
    tempoMaximoResposta ??= padraoTempoMaximoResposta;
    tempoMaximoSolucao ??= padraoTempoMaximoSolucao;
  ...
  void calcularTempos() {
    print('calculando-SLA||$atendimento||${data.toIso8601String()}||');
  ...
  double get percentualDeResposta {
    calcularTempos();
    print('percentualDeResposta');
    if (tempoDeResposta == null) {
      print('tempoDeResposta == null');
      return 0;
    }
    final tempoTotal = horaTermino.difference(horaInicio);
    print(tempoDeResposta!.inSeconds / tempoTotal.inSeconds);
  ...
  double get percentualDeSolucao {
    calcularTempos();
    print('percentualDeSolucao');
    if (tempoDeSolucao == null) {
      print('tempoDeSolucao == null');
      return 0;
    }
    final tempoTotal = horaTermino.difference(horaInicio);
    print(tempoDeSolucao!.inSeconds / tempoTotal.inSeconds);
  ...
  double get percentualCumprido {
    calcularTempos();
    print('percentualCumprido');
    if (tempoDeAtendimento == null || tempoDeAtendimento == Duration.zero) {
      print(
          'tempoDeAtendimento == null || tempoDeAtendimento == Duration.zero');
      return 0;
    }
